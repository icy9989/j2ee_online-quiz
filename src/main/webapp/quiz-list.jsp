...
%>
		...
		<nav class="navbar navbar-expand-md mb-4">
			...
			<div id="navbarcollapse"
				...
				<ul class="navbar-nav">
					...
					<li class="nav-item"><a href="instructor-dashboard"
						class="nav-link mx-3">Students</a></li>
					<li class="nav-item"><a href="instructor-dashboard"
						class="nav-link mx-3">Courses</a></li>
				</ul>
				<form action="logout">
	...
	<section>
		<div class="row justify-content-between align-items-center m-3">
			<div
				class="col-xl-3 col-lg-4 col-md-5 col-sm-6 mb-4 d-flex justify-content-sm-start justify-content-between">
				<button class="btn btns btn-sm margin-right" type="button"
					name="back" onclick="history.back()">
					<i class="fas fa-arrow-alt-circle-left"></i> Back
				</button>
				<a href="quiz-form?course_code=${course_code}" type="button"
					class="btn btn-outline btn-sm btns">+ Create New Quiz</a>
			</div>
			<div class="col-xl-3 col-lg-4 col-md-5 col-sm-5 mb-4">
				<div class="input-group">
					<input type="search" name="search" id="search"
						class="form-control form-control-sm rounded-0"
						onkeyup="tableSearch()" placeholder="Search..." />
					<button type="button" class="btn btn-sm btns"
						onclick="tableSearch()">
		...
		<div class="bg-white mx-4 px-3">
			<div class="table-responsive-lg">
				<table id="myTable" class="table m-2">
					...
					<tbody>
						<c:forEach var="q"
							items="<%= request.getAttribute(\"quizList\") %>" varStatus="i">
							<tr>
								<td><c:out value="${i.count}" /></td>
								<td><c:out value="${q.name}" /></td>
								<td><c:out value="${q.date}" /></td>
								<td><c:out value="${q.startTime}" /></td>
								<td><c:out value="${q.total_question}" /></td>
								<td><c:out value="${q.total_mark}" /></td>
								<td><c:out value="${q.duration}" /> min</td>
								<td><a href="quiz-detail?id=<c:out value='${q.id}' />"><i
										class="fas fa-edit text-info"></i></a></td>
								<td><a href="quiz-result?id=${q.id}&code=${course_code}"><span

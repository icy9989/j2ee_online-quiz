...
<body>
						...
						<div class="col-xl-2 col-lg-3 col-md-5 fixed-top sidebars">
							...
							<ul class="mt-5 lists">
								<li class="listitems dashboards actives"><a
									href="instructor-dashboard"><i class="fas fa-home mx-3"></i>Dashboard</a></li>
								<li class="listitems students"><a href="#"><i
										class="fas fa-user-graduate mx-3"></i>Students</a></li>
								<li class="listitems courses"><a href="#"><i
										class="fas fa-laptop-code mx-3"></i>Courses</a></li>
							</ul>

							<div class="positioncenters m-5">
								<a href="logout" class="btn btn-info text-white btns">Log
									out</a>
							</div>
	...
	<section class="container-fluid">
		<div class="row">
			...
			<div id="dashboard" class="col-xl-7 col-lg-5 col-sm-7 ms-auto p-4">
				...
				<c:if test="${quizActionDoneMsg != null}">
					<div class="alert g-alerts">
						<p class="text-secondary">
							<strong>Success!</strong> Successfully ${quizActionDoneMsg}
						</p>
					</div>
				</c:if>

				<c:if test="${quizActionFailedMsg != null}">
					<div class="alert alert-danger">
						<p class="text-secondary">
							<strong>Failed!</strong> ${quizActionFailedMsg}
						</p>
					</div>
				</c:if>
				...
				session.removeAttribute("quizActionDoneMsg");
				session.removeAttribute("quizActionFailedMsg");
				%>
			...
			<!-- Start Student -->
			<div id="student" class="col-xl-7 col-lg-5 col-sm-7 ms-auto p-4">
				<p class="fw-bold text-secondary text-uppercase">Student</p>
				<div class="row">
					<c:forEach var="c"
						items="<%=session.getAttribute(\"instructor-course-list\")%>"
						varStatus="loop">
						<div class="col-xl-6 px-5 my-3 newcards">
							<a href="instructor-students?course_code=${c.code }">
								<div class="card px-4">
									<div class="card-body"  data-toggle="tooltip" data-placement="top" title="${c.courseName}">
										<div class="d-flex justify-content-between align-items-center">
											<i class="fas fa-user-graduate fa-3x"></i>
											<div class="text-secondary">
												<h5 class="display-courses">${c.courseName}</h5>
												<c:choose>
													<c:when test="${loop.count == 1}">
														<h5 class="pt-2">${course_std_count1}</h5>
													</c:when>
													<c:otherwise>
														<h5 class="pt-2">${course_std_count2}</h5>
													</c:otherwise>
												</c:choose>
											</div>
										</div>
			...
			<div id="course" class="col-xl-7 col-lg-5 col-sm-7 ms-auto p-4">
				...
				<div class="row ">
					<c:forEach var="c"
						...
						varStatus="loop">
						<div class="col-xl-6 px-5 my-3 newcards">
							<a href="quizzes?course_code=${c.code }">
								<div class="card px-4">
									<div class="card-body" data-toggle="tooltip" data-placement="top" title="${c.courseName}">
										<div class="d-flex justify-content-between align-items-center">
											<i class="fas fa-book fa-3x"></i>
											<div class="text-secondary">
												<h5>${c.code}</h5>
												<h5 class="pt-2 display-courses">${c.courseName}</h5>
											</div>
										</div>
									</div>
								</div>
			...
			<div class="col-xl-3 col-lg-4 col-sm-5 p-5">
				<div class="calendarcards bg-white">
					...
					<div class="p-3 text-secondary datecards positioncenters">
						<p id="month" class="monthfonts">July</p>
						<p class="datefonts">
	...
	<script src="http://localhost:8080/online_quiz/js/app.js"
		type="text/javascript"></script>
	<script
		src="http://localhost:8080/online_quiz/js/instructor-dashboard.js"
		type="text/javascript"></script>
